...
export class NotificationManager {
	...
	async notify(notification: FridayNotification, channelNames?: string[]): Promise<void> {
		const targets = channelNames
			? (channelNames.map((n) => this.channels.get(n)).filter(Boolean) as NotificationChannel[])
...
export class WebhookChannel implements NotificationChannel {
	...
	async send(notification: FridayNotification): Promise<void> {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Webhook failed: ${response.status} ${response.statusText} — ${body.slice(0, 200)}`,
				);
			}
		} finally {
...
export class SlackChannel implements NotificationChannel {
	...
	async send(notification: FridayNotification): Promise<void> {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Slack webhook failed: ${response.status} ${response.statusText} — ${body.slice(0, 200)}`,
				);
			}
		} finally {
			clearTimeout(timeout);
