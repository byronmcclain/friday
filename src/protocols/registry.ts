...
export class ProtocolRegistry {
	...
	get(nameOrAlias: string): FridayProtocol | undefined {
		return (
			this.protocols.get(nameOrAlias) ?? this.protocols.get(this.aliases.get(nameOrAlias) ?? "")
		);
	}
	...
	search(query: string): FridayProtocol[] {
		...
		return this.list().filter(
			(p) =>
				p.name.toLowerCase().startsWith(q) || p.aliases.some((a) => a.toLowerCase().startsWith(q)),
		);
	}
