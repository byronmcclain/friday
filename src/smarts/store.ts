...
export class SmartsStore {
	...
	async create(entry: Omit<SmartEntry, "filePath">): Promise<SmartEntry> {
		...
		if (existing && existing.name !== entry.name) {
			throw new Error(
				`SMART entry name "${entry.name}" collides with existing "${existing.name}" (both sanitize to "${safeName}")`,
			);
		}
	...
	async update(
		name: string,
		content: string,
		opts?: { tags?: string[]; confidence?: number },
	): Promise<void> {
		const key = this.sanitizeName(name);
		...
		const newId = await this.memory.embed(SMARTS_NAMESPACE, embeddingContent, {
			name: existing.name,
		});
