...
describe("manage_rhythm tool", () => {
	...
	test("update operation modifies prompt action", async () => {
		const rhythm = store.create({
			...
			enabled: true,
			origin: "friday",
			action: { type: "prompt", prompt: "old prompt" },
			nextRun: new Date(),
			clearance: [],
		});
		const tool = createManageRhythmTool(store);
		...
		expect((updated.action as { type: "prompt"; prompt: string }).prompt).toBe(
	...
	test("update operation modifies tool action", async () => {
		const rhythm = store.create({
			...
			enabled: true,
			origin: "friday",
			action: { type: "tool", tool: "gmail.search", args: { query: "is:unread" } },
			nextRun: new Date(),
			clearance: [],
		});
		const tool = createManageRhythmTool(store);
	...
	test("update operation modifies protocol action", async () => {
		const rhythm = store.create({
			...
			enabled: true,
			origin: "friday",
			action: { type: "protocol", protocol: "env", args: { rawArgs: "status" } },
			nextRun: new Date(),
			clearance: [],
		});
		const tool = createManageRhythmTool(store);
	...
	test("update operation rejects invalid action_config JSON for tool type", async () => {
		const rhythm = store.create({
			...
			enabled: true,
			origin: "friday",
			action: { type: "tool", tool: "gmail.search", args: {} },
			nextRun: new Date(),
			clearance: [],
		});
		const tool = createManageRhythmTool(store);
