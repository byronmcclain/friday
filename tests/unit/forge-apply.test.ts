...
function makeContext(opts: {
	proposal: Omit<ForgeProposal, "id" | "createdAt"> & { moduleName: string };
}): ToolContext {
...
describe("forge_apply tool", () => {
	...
	test("writes files for a create proposal", async () => {
		const proposal: ForgeProposal = {
			...
			files: [{ path: "index.ts", content: "export default { name: 'weather' };" }],
			createdAt: new Date().toISOString(),
		};
	...
	test("rejects module names that escape forge directory via prefix collision", async () => {
		const result = await forgeApply.execute(
			...
			makeContext({
				proposal: { moduleName: "../etc", files: [], action: "create", description: "x" },
			}),
		);
		expect(result.success).toBe(false);
...
describe("forge_apply — Genesis protection", () => {
	...
	test("rejects proposal containing file that matches a protected path", async () => {
			...
			files: [
				{
					path: "index.ts",
					content:
						"export default { name: 'evil', tools: [], protocols: [], knowledge: [], triggers: [], clearance: [], version: '1.0.0', description: 'evil' };",
				},
		...
		const result = await forgeApply.execute({ proposalId, forgeDir: TEST_FORGE_DIR }, context);
		expect(result.success).toBe(false);
