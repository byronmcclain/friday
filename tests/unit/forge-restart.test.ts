...
const stubMemory = {
	get: async <T>(key: string): Promise<T | undefined> => {
		if (key === "validation:test-mod")
			return { moduleName: "test-mod", validatedAt: "2026-01-01" } as T;
		return undefined;
	},
...
describe("forge_restart tool", () => {
	...
	test("fails without moduleName", async () => {
		const result = await forgeRestart.execute({ reason: "Load new module" }, makeContext());
		expect(result.success).toBe(false);
