...
describe("buildGrokRealtimeUrl", () => {
	...
	test("defaults to grok-voice-latest query param", () => {
		...
		expect(buildGrokRealtimeUrl()).toBe(`${GROK_REALTIME_URL}?model=${GROK_VOICE_MODEL_DEFAULT}`);
	});
	...
	test("FRIDAY_VOICE_MODEL overrides default", () => {
		...
		expect(buildGrokRealtimeUrl()).toBe(`${GROK_REALTIME_URL}?model=grok-voice-think-fast-2.0`);
	});

	test("explicit model option wins over env", () => {
		process.env.FRIDAY_VOICE_MODEL = "from-env";
		expect(buildGrokRealtimeUrl({ model: "explicit" })).toBe(`${GROK_REALTIME_URL}?model=explicit`);
	});
